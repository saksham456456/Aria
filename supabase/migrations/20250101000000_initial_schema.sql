...
-- ROW LEVEL SECURITY
-- Architecture note:
--   Server-side routes use SUPABASE_SERVICE_ROLE_KEY which bypasses RLS.
--   Browser client uses NEXT_PUBLIC_SUPABASE_ANON_KEY which is subject to RLS.
--   RLS policies below protect direct browser queries using a custom x-user-id
--   header (passed via Supabase global.headers in getSupabaseBrowser()).
-- ============================================================
...
-- Sessions: readable by participants of that session
...
-- Messages: readable + insertable by session participants
CREATE POLICY "messages_read_by_participant"
  ON messages FOR SELECT USING (
    EXISTS (SELECT 1 FROM participants p WHERE p.session_id = messages.session_id
      AND p.app_user_id = (current_setting('request.headers', true)::json->>'x-user-id'))
  );
...
CREATE POLICY "aria_events_read_by_participant"
  ON aria_events FOR SELECT USING (
    EXISTS (SELECT 1 FROM participants p WHERE p.session_id = aria_events.session_id
      AND p.app_user_id = (current_setting('request.headers', true)::json->>'x-user-id'))
  );
...
CREATE POLICY "learning_gaps_read_by_participant"
  ON learning_gaps FOR SELECT USING (
    EXISTS (SELECT 1 FROM participants p WHERE p.session_id = learning_gaps.session_id
      AND p.app_user_id = (current_setting('request.headers', true)::json->>'x-user-id'))
  );

-- Transcript: readable + insertable by participants
CREATE POLICY "transcript_read_by_participant"
  ON transcript_segments FOR SELECT USING (
    EXISTS (SELECT 1 FROM participants p WHERE p.session_id = transcript_segments.session_id
      AND p.app_user_id = (current_setting('request.headers', true)::json->>'x-user-id'))
  );
CREATE POLICY "transcript_insert_by_participant"
  ON transcript_segments FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM participants p WHERE p.session_id = transcript_segments.session_id
      AND p.app_user_id = (current_setting('request.headers', true)::json->>'x-user-id'))
  );
