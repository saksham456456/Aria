...
export async function POST(request: Request) {
  try {
    ...
    const { data: session, error: sessionError } = await supabaseServer
      .from('sessions')
      .insert({ classroom_id: classroom.id, status: 'active' })
      .select('id')
      .single();
    ...
    if (participantError) throw new Error(`Failed to register teacher: ${participantError.message}`);

    return successResponse({ sessionId: session.id, joinCode });
