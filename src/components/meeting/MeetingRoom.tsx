...
function MeetingRoomInner({ sessionId, appUserId }: { sessionId: string; appUserId: string }) {
  ...
  const handleGenerateSummary = useCallback(async (topics: string) => {
    ...
    try {
      if (appUserId) {
        const res = await fetch('/api/summary/custom', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-user-id': appUserId },
          body: JSON.stringify({ sessionId, topics, saveToDb: false })
        });
        const json = await res.json();
        if (json.success) {
          setSummaryPreview(json.data.summary);
  ...
  const handleApproveSummaryAndEnd = useCallback(async () => {
    ...
    try {
      ...
      await supabase.from('session_summaries').upsert({
        ...
        overview: summaryPreview.overview,
        topics_covered: summaryPreview.topics_covered,
        recommendations: summaryPreview.key_points.join('\n\n'),
      ...
      console.error('Failed to end class and save summary', err);
    }
  }, [summaryPreview, pauseAria, appUserId, sessionId, handleLeave]);
    ...
    <div className="flex flex-col h-screen bg-surface-0 text-white overflow-hidden">
      ...
      {summaryPreview && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[120] p-4">
          <div className="bg-surface-1 border border-surface-3 rounded-2xl p-6 max-w-lg w-full shadow-2xl max-h-[80vh] flex flex-col">
            <h2 className="text-xl font-bold text-white mb-2">Review Class Summary</h2>
            <p className="text-sm text-slate-400 mb-6">This summary will be shown to all students when you end the class.</p>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 mb-6 pr-2">
              <div className="bg-surface-2 rounded-xl p-4 border border-surface-3">
                <h3 className="text-xs font-semibold text-slate-300 uppercase tracking-widest mb-2">Overview</h3>
                <p className="text-sm text-slate-300 leading-relaxed">{summaryPreview.overview}</p>
              </div>
              
              <div className="bg-surface-2 rounded-xl p-4 border border-surface-3">
                <h3 className="text-xs font-semibold text-slate-300 uppercase tracking-widest mb-2">Key Points</h3>
                <ul className="list-disc list-inside text-sm text-slate-300 leading-relaxed space-y-1">
                  {summaryPreview.key_points.map((pt: string, i: number) => (
                    <li key={i}>{pt}</li>
                  ))}
                </ul>
              </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-surface-3">
              ...
              <button
                onClick={handleApproveSummaryAndEnd}
                className="px-5 py-2 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-500 rounded-xl transition-colors shadow-lg shadow-emerald-900/20"
              >
                Approve & End Class
              </button>
            </div>
